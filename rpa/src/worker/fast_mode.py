...
from db.repository import BatchUpserterBulk, SyncStats, get_existing_modifications_by_ids, normalize_bps_date
        ...
        async with sem:
            ...
            if res:
                ...
                sync_state.progress.phase_label = (
                    f"🌏 Fetching: {total_current} records... (User: {f_user.get('label')})"
                )
            return res
    ...
    try:
        ...
        for m in unique_metadata:
            ...
            if rec_id in existing_mods and norm_db == norm_api and norm_db != "":
                ...
                upserter.add({"_id": rec_id, "assignment": m, "responses": [], "_survey_config_id": survey_config_id})
