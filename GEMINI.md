...
14. **FortiGate Idle Tunnel Death (Silent Drop)**: FortiGate BPS memiliki idle timeout **60 menit**. Jika tidak ada traffic aktif, FortiGate akan **silently drop** semua paket tanpa menutup koneksi TCP — tun0 tetap `UP` di kernel tapi traffic tidak mengalir. Gejalanya: TX/RX ratio sangat timpang (TX jauh lebih besar dari RX), dan `curl` ke fasih-sm gagal dengan `Connection reset by peer` di semua nilai MTU. **Solusi permanen**: gunakan `--dpd 30` pada OpenConnect agar ada keepalive ping setiap 30 detik. MTU bukan penyebab masalah ini.
