...
## Sync Engine Improvements & Stability Fixes (July 2026)

1. **UUID Extraction Bug Fix:** Resolved an issue in the detail extraction loop (`ultimate_engine.py`) where the assignment ID was parsed as `None` because the BPS API response key for detail objects is `_id` instead of `id`. Checked both keys to ensure correct UUID propagation.
2. **PostgreSQL Bulk Insert Mapping Fix:** Fixed a WAF-related camelCase vs snake_case mapping issue in the database repository. Swapped empty-string UUID defaults to `None` to prevent `psycopg2.errors.InvalidTextRepresentation: invalid input syntax for type uuid: ""` crashes on bulk insert.
3. **Refactored API Client:** Replaced standard requests/aiohttp with `curl_cffi` to mimic Google Chrome's JA3 fingerprint and enforce HTTP/1.1 to bypass F5 WAF limits.
4. **WAF Cookie Stabilization:** Replaced the fast early-polling cookie loop in `auth.py` with a structured wait for landing page Angular elements (`SUCCESS_SELECTORS`) followed by a 3-second sleep to allow dynamic WAF cookies (`_cspm_`) to fully populate.
5. **Accept Header Normalization:** Changed the default `Accept` header in `api_client.py` from the compound `"application/json, text/plain, */*"` (which triggered a signature violation on F5 WAF) to strictly `"Accept": "application/json"`.
6. **Removing Root Bootstrapping:** Removed the redundant session-warming GET request to the root HTML page `/` in `api_client.py`. Sending API-specific headers (`Accept: json`) to an HTML page caused a header-content mismatch that flagged the session.
7. **Supervisor Filters Fix in Pencacah Rotation (July 2026):** Fixed a bug in `job_runner.py` where `pengawas_id` was omitted when constructing search filters under the `pencacah` rotation mode, which caused supervisor-held assignments to be missed. Fixing this pulled supervisor assignments and successfully increased the local dataset from 60 to 64.
8. **BPS Datatable API Region-Only Crash (July 2026):** Discovered that calling `datatable-all-user-survey-periode` without a `currentUserId` (Wilayah Saja mode) triggers an HTTP 500 error on the BPS backend. Therefore, user-slicing is mandatory, and any unassigned assignments (difference between Remote Target 94 and Scoped Metadata 64) cannot be retrieved until they are assigned to a user.
