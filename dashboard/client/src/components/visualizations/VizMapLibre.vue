...
const styleObj: any = {
	...
	sources: {
		"carto-dark": {
			...
			tileSize: 256,
			attribution:
				'&copy; <a href="http://www.openstreetmap.org/copyright">OSM</a>, &copy; <a href="https://carto.com/attributions">CARTO</a>',
		},
	},
	layers: [
		{
	...
	map.on("load", () => {
		...
		map.addLayer({
			...
			paint: {
				"circle-radius": [
					...
					["zoom"],
					4,
					["max", 4, ["min", 12, ["to-number", ["get", "metric_count"], 1]]],
					14,
					["max", 8, ["min", 30, ["*", 3, ["to-number", ["get", "metric_count"], 1]]]],
				],
		...
		map.on("click", "points-layer", (e: any) => {
			...
			const matchedRule = colorRules.find(
				(r: any) => r.value && String(r.value) === String(p.color_val),
			);
