...
export function useVizForm(
	...
	const newViz = ref({
		...
		config: {
			...
			colorBy: string;
			colorRules: any[];
			popupFields: { column: string; label: string }[];
			metrics: any[];
			[key: string]: any;
		},
	});
	...
	function filterMeasure(val: string, update: (cb: () => void) => void) {
		update(() => {
			...
			filteredMeasureColumns.value = measureColumns.value.filter((v: any) =>
	...
	function filterDimension(val: string, update: (cb: () => void) => void) {
		update(() => {
			const needle = val.toLowerCase();
			filteredDimensionColumns.value = dimensionColumns.value.filter((v: any) =>
				v.label.toLowerCase().includes(needle),
			);
		});
	}
	...
	function openAddDialog() {
		if (
			!labelSchema.value ||
			!labelSchema.value.columns ||
			labelSchema.value.columns.length === 0
		) {
			$q.notify({
				type: "warning",
				message: "Belum ada kolom yang tersedia untuk dibuat visualisasi.",
			});
			return;
		}
	...
	function openEditDialog(viz: any) {
		...
		if (!normalizedConfig.metrics || normalizedConfig.metrics.length === 0) {
			if (viz.chartType === "scorecard") {
				normalizedConfig.metrics = [
					{
						id: "m0",
						type: "regular",
						label: normalizedConfig.label || normalizedConfig.metricColumn,
						column: normalizedConfig.metricColumn,
						aggregation: normalizedConfig.aggregation,
					},
				];
				...
				normalizedConfig.metrics = [
					{
						id: "m0",
						type: "regular",
						label: normalizedConfig.label || normalizedConfig.yColumn,
						column: normalizedConfig.yColumn,
						aggregation: normalizedConfig.aggregation,
					},
				];
			} else {
	...
	async function loadPreview() {
		...
		if (isMap) {
			...
			const invalidM = newViz.value.config.metrics.find(
				(m: any) => m.type === "regular" && !m.column,
			);
