...
export const getAuthContext = async (request: Request) => {
	...
	const sessionToken =
		cookies["__Secure-cdc_auth.session_token"] || cookies["cdc_auth.session_token"];

	if (!sessionToken) {
		console.warn(
			`[Auth] No session token found in cookies. Available: ${Object.keys(cookies).join(", ")}`,
		);
...
export const authMiddleware = new Elysia().derive({ as: "global" }, async ({ request }) => {
...
export const requireAdmin = (app: Elysia) =>
	app.use(authMiddleware).onBeforeHandle(({ user, roles, set }: any) => {
		if (!user) {
			set.status = 401;
			return { error: "Unauthorized" };
		}
