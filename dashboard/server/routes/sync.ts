...
export const syncRoutes = new Elysia({ prefix: "/api/surveys" })
	...
	.post("/:id/sync", async (ctx: any) => {
		...
		const [survey] = await db.select().from(surveyConfigs).where(eq(surveyConfigs.id, params.id));
		...
		log.info("Triggering sync in RPA engine", {
			survey_id: survey.id,
	...
	.get("/sync/status", async (ctx: any) => {
		...
		try {
			...
			log.error("Failed to fetch RPA sync status", {
				error: e.message,
	...
	.get("/vpn/status", async (ctx: any) => {
		...
		try {
			...
			log.error("Failed to fetch VPN status from RPA", {
				error: e.message,
				rpa_url: `${RPA_URL}/vpn/check`,
			});
	...
	.delete("/vpn/cookie", async () => {
		await db.delete(systemSettings).where(eq(systemSettings.key, "vpn_cookie"));
	...
	.post("/vpn/auto-fetch", async (ctx: any) => {
		...
		try {
			...
			log.error(`VPN Auto-fetch connection failed: ${e.message}`, {
				error: e.message,
				stack: e.stack,
			});
			set.status = 503;
		...
		if (!statusRes.connected) {
			...
			try {
				...
				if (dbErr.message?.includes("does not exist")) {
					...
					log.error("Database Schema Mismatch (UUID Cast Error). Please run manual migration.", {
						error: dbErr.message,
					});
				} else {
			...
			if (!survey) {
				...
				if (process.env.VPN_USER && process.env.VPN_PASS) {
					log.info("No active survey found, but using Master SSO (VPN_USER) for bootstrap...", {
				...
				log.warn(
					"No active survey found and no Master SSO (VPN_USER) configured. Cannot auto-pilot.",
				);
				return;
			}
