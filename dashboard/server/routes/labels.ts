...
import { db } from "../db";
import { assignments, labelData, labelSchemas } from "../db/schema";
...
export const labelsRoutes = new Elysia({ prefix: "/api/surveys" })
	...
	.get("/:id/labels/template", async ({ params, set }) => {
		...
		const extraColumns: string[] = schema ? (schema.columns as ColumnDef[]).map((c) => c.name) : [];
		...
		const wsData = [
			...
			...codeRows.map((r) => {
				...
				return [r.codeIdentity, ...extraColumns.map((col) => existing[col] ?? "")];
			}),
		];
	...
	.post("/:id/labels/upload", async ({ params, body, set }) => {
		try {
			...
			const validRows = data.filter(
				(row: any) => row.code_identity && String(row.code_identity).trim(),
			);
			...
			const toInsert = validRows.map((row: any) => {
				...
				for (const col of allColumns) {
					...
					if (val !== null && val !== undefined && String(val).trim() !== "") {
						...
						rowData[col] =
							colDef?.type === "measure" && !isNaN(Number(val)) ? Number(val) : String(val).trim();
					}
				}
				return {
					surveyConfigId: params.id,
